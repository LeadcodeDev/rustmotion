...
- `codeblock` / `terminal` auto-scroll when content exceeds the box set via `style.width`/`style.height` — leave `auto_scroll: true` (default). There is no root `size` field on these components.
