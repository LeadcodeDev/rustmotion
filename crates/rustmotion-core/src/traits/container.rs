...
/// Marker trait for components that contain children.
/// The actual `children()` method lives on each concrete container type
/// (returning `&[ChildComponent]`), not on this trait, to avoid a circular
/// dependency between core and components.
pub trait Container {}
