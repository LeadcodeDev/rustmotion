...
**Merging behavior:** Base `animation` effects (in `style`) play from the start. Timeline step effects are merged additively when they activate. Multiple steps can overlap.
...
```
Don't use timeline for initial entrance — use `animation` inside `style` directly. Timeline is for subsequent phases.
